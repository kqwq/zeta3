...
import { onPeerConnect, onPeerData, onPeerDisconnect, cheatDetector } from './game.js'
...
async function createNewPeer(initialOffer, offerLineNumber, s2) {
  ...
  peer.on('data', data => {
    ...
    if (data[0] == "~") { // If intended to a specific peer
      ...
      if (isCheating) {
        ...
        peerData[peerObjIndex].peer.send("cheating")
        peerData[peerObjIndex].peer.destroy()
        return
      }
      ...
      if (data.slice(1, 5) == "room") {
        ...
        for (let peerInfo of peerData) {
          if (peerInfo.scratchpad.id != srcId) {
            peerInfo.peer.send(`~${data.slice(5)}${srcId}`)
          }
        }
        return
      }
  ...
  let peerObj = {
    ...
    scratchpad: isKASource ? s2 : { id: getRandomString() }, // Khan Academy scratchpad data (includes program id and kaid)
    ipInfo: {
      ip: ipAddress
